...
export default async function LeaderboardPage() {
  ...
  const [weekly, allTime] = await Promise.all([
    ...
    caller.leaderboard.top({ period: 'allTime', limit: 10 }),
  ]);
  return (
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <section>
          ...
          <ol className="mt-2 divide-y rounded border">
            ...
            {weekly.items.length === 0 && <li className="p-3 text-sm text-gray-600">No entries yet</li>}
          </ol>
        </section>
        <section>
          <h2 className="text-lg font-semibold">All‑time</h2>
          <ol className="mt-2 divide-y rounded border">
            {allTime.items.map((it) => (
              <li key={it.userId} className="flex items-center justify-between p-3">
                <div className="flex items-center gap-3">
                  <div className="w-6 text-right tabular-nums">{it.rank}.</div>
                  <div>{it.label}</div>
            ...
            {allTime.items.length === 0 && <li className="p-3 text-sm text-gray-600">No entries yet</li>}
          </ol>
        </section>
      </div>
    </main>
